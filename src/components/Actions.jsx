...
const Actions = ({ post }) => {
  ...
  const handleToggleLike = async () => {
    ...
    try {
      ...
      if (!liked) {
        ...
        // remove the currentUserId from post.likes array
        const updatedPosts = posts.map((p) => {
          if (p._id === post._id) {
  ...
  const handleReply = async () => {
    if (!user) {
      toast.error("You must to login to reply a post");
    }
    if (isReplying) return;
    setIsReplying(true);
    try {
      const response = await axios.put(`/api/v1/posts/reply/${post._id}`, {
        text: reply,
      });
      ...
      const updatedPosts = posts.map((p) => {
        if (p._id === post._id) {
          return { ...p, replies: [...p.replies, response.data.reply] };
        }
        return p;
      });
      setPosts(updatedPosts);
